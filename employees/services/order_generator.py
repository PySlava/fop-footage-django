...
def format_full_name_genitive(full_name):
    """Переводит ПІБ у родовий відмінок (наприклад, Іванова Івана Івановича)"""
...
def generate_hiring_order_docx(employee, order_num, order_date, start_date):
    ...
    # Шапка: ФОП ПЕТРЕНКО ПЕТРО ПЕТРОВИЧ
    ...
    # Мета-рядок (дата, місто/крапки, номер)
    ...
    # Ліва комірка: від 03.09.2026 р.
    cell_left = row.cells[0]
    set_cell_margins(cell_left)
    p_left = cell_left.paragraphs[0]
    p_left.alignment = WD_ALIGN_PARAGRAPH.LEFT
    p_left.add_run(f"від {order_date.strftime('%d.%m.%Y')} р.")
    ...
    # Тема: Про призначення Іванова І. І.
    ...
    p_body.add_run(f"ПРИЗНАЧИТИ з {start_date.strftime('%d.%m.%Y')} року {genitive_name} {pos_str}")
...
def generate_dismissal_order_docx(employee, order_num, order_date, dismissal_date):
    ...
    # Тема: Про звільнення Іванова І. І.
    p_sub = doc.add_paragraph()
    p_sub.paragraph_format.space_before = Pt(18)
    p_sub.paragraph_format.space_after = Pt(18)
    p_sub.paragraph_format.line_spacing = 1.15
    ...
    p_body.add_run(f"ЗВІЛЬНИТИ {dismissal_date.strftime('%d.%m.%Y')} року {genitive_name}, {pos_str} за згодою сторін відповідно до пункту 1 статті 36 КЗпП України.")
